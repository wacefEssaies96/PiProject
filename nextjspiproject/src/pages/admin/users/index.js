...
function Index({ users }) {
  ...
  const showRoleFilter = (filter) => {
    ...
    setAdmin(false);
    setClient(false);
    if (filter == 'allRoles') setAllRoles(true);
    if (filter == 'admin') setAdmin(true);
    if (filter == 'client') setClient(true);
    if (filter == 'doctor') setDoctor(true);
  };
  ...
  const deleteOneUser = async (id) =>
    deleteData(`${process.env.backurl}/api/users/${id}`).then(refresh);

  return (
    <Container>
      <h1>List of Users</h1>
      <Link className="btn btn-outline-success" href={`/admin/users/create`}>
        Create new user
      </Link>
      <hr />
      <Button
        variant="outline-success"
        onClick={() => showRoleFilter('allRoles')}
      >
        All
      </Button>
      &nbsp;
                  ...
                  <td key={user._id}>
                    ...
                    {user.role == 'DOCTOR' && (
                      <Form onSubmit={approve} encType="multipart/form-data">
                        ...
                        ></input>
                        <Button type="submit" disabled={user.account_Verified}>
                          approve
                        </Button>
                      </Form>
                    )}
